...
// Check if the user is an admin and if valid community_id and user_to_ban_id are provided
if (isset($_GET['community_id'], $_GET['user_to_ban_id'])) {
    $community_id = $_GET['community_id'];
    $user_to_ban_id = $_GET['user_to_ban_id'];
    ...
    if ($admin_data && $admin_data['role'] === 'admin') {
        ...
        $ban_query = "INSERT INTO ban_members (community_id, user_id) VALUES (?, ?)";
        $stmt = $conn->prepare($ban_query);
        $stmt->bind_param("ii", $community_id, $user_to_ban_id);
        $stmt->execute();
